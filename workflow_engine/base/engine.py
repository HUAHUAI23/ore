...
class BaseWorkflowEngine(ABC):
    ...
    def __init__(self, workflow_config: BaseWorkflowConfig) -> None:
        """初始化工作流引擎基类"""
        # 基本配置 - 所有工作流都有的共同字段
        self.workflow_id: str = workflow_config['workflow_id']
        self.workflow_name: str = workflow_config['name']
        self.workflow_type: str = workflow_config.get('type') or 'generic'
    ...
    def _mark_node_completed(self, node_id: str, result: Any) -> None:
        ...
        self._log_event(EventType.NODE_SUCCESS.value, {"node_id": node_id, "result": result})
    
    def _mark_node_failed(self, node_id: str, error: Exception) -> None:
        """标记节点失败"""
        self.failed_nodes.add(node_id)
        self.node_results[node_id] = f"ERROR: {error}"
        self._log_event(EventType.NODE_FAILED.value, {"node_id": node_id, "error": str(error)})
